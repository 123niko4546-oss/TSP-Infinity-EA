...
public:
      ...
      if(signal.key != m_lastSignalKey)
      {
         ...
         if(m_enabled)
         {
            Print("[TSP4][SIGNAL_NEW] key=",
                  ...
                  " created=",
                  TimeToString(signal.created, TIME_DATE|TIME_MINUTES));
         }
      }
   }

   void Stage(STSP4_Signal &signal,
              string stage,
              string reason)
   {
      ...
      if(stage == "WAIT_H1")
      {
         if(m_signalFirstSeen > 0)
         {
            int bars = iBarShift(Symbol(),
                                 PERIOD_H1,
                                 m_signalFirstSeen,
                                 false);
            if(bars >= 0)
   ...
   void EntrySnapshot(STSP4_Signal &signal,
                      string entryReason)
   {
      if(!m_enabled)
         return;
      ...
      Print("[TSP4][ENTRY_SNAPSHOT] key=",
            signal.key,
            " direction=",
            TSP4_DirectionText(signal.direction),
            " H1Time=",
            TimeToString(iTime(Symbol(), PERIOD_H1, 0),
            ...
            " SAR0=",
            DoubleToString(sar0, Digits),
            " SAR1=",
            DoubleToString(sar1, Digits),
            " High0=",
            DoubleToString(iHigh(Symbol(), PERIOD_H1, 0), Digits),
            " Low0=",
            DoubleToString(iLow(Symbol(), PERIOD_H1, 0), Digits),
            " detail=",
            entryReason);
   }
};
